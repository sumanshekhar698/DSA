...
public class _496NextGreaterElementI {
    ...
    static public int[] nextGreaterElementUsingStack(int[] nums1, int[] nums2) {
//        //O(n) = n+m
        int[] res = new int[nums1.length];
        Arrays.fill(res, -1);//to avoid flag logic

        HashMap<Integer, Integer> map = new HashMap<>();//nums[i]:i
        for (int i = 0; i < nums1.length; i++) {//enriching map from nums1
            map.put(nums1[i], i);
        }


        Stack<Integer> stack = new Stack<>();//STACK will contain only those elements whose ngl has to taken out
        //ie the elements in nums1

        for (int i = 0; i < nums2.length; i++) {
            int current = nums2[i];
            while (!stack.isEmpty() && current > stack.peek()) {
                Integer popped = stack.pop();//all the popped elements have the ngl as the current element
...
//        //O(n) = n+m
        ...
        ArrayDeque<Integer> stack = new ArrayDeque<>();//STACK it wil contain only those element whose ngl has to taken out
        //ie the elements in nums1
